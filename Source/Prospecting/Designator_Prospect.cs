...
namespace Prospecting
{
	...
	public class Designator_Prospect : Designator
	{
		...
		{
			this.defaultLabel = "Prospecting.DesignatorProspect".Translate();
			this.icon = ContentFinder<Texture2D>.Get("UI/Designators/Prospecting", true);
			this.defaultDesc = "Prospecting.DesignatorProspectDesc".Translate();
		...
		public override AcceptanceReport CanDesignateCell(IntVec3 c)
		{
			...
			if (base.Map.designationManager.DesignationAt(c, this.MineDesig) != null)
			{
				return AcceptanceReport.WasRejected;
			}
			...
			List<IntVec3> chkcells = GenAdjFast.AdjacentCellsCardinal(c);
			bool entry = false;
			if (chkcells.Count > 0)
			{
				foreach (IntVec3 chkcell in chkcells)
				{
					if (chkcell.InBounds(base.Map) && (chkcell.Standable(base.Map) || chkcell.Walkable(base.Map)))
					{
						entry = true;
						break;
					}
		...
		public override AcceptanceReport CanDesignateThing(Thing t)
		{
			...
			if (base.Map.designationManager.DesignationAt(t.Position, this.MineDesig) != null)
			{
				return AcceptanceReport.WasRejected;
			}
			List<IntVec3> chkcells = GenAdjFast.AdjacentCellsCardinal(t.Position);
			bool entry = false;
			if (chkcells.Count > 0)
			{
				foreach (IntVec3 chkcell in chkcells)
		...
		{
			List<IntVec3> cells = GenRadial.RadialCellsAround(root, (float)near, true).ToList<IntVec3>();
			if (cells.Count > 0)
			{
